...
function findUnused(srcFiles, resolver) {
  ...
  srcFiles.forEach(path => {
    const objectWithPath = name => { return { name, path }; };
    const src = resolver ? resolver(path) : fs.readFileSync(path).toString();
    const parseTree = gonzales.parse(src, {syntax: 'scss'});

    // Variables
    declaredVars.push(...gatherDeclaredVars(parseTree).map(objectWithPath));
    gatherUsedVars(parseTree).forEach(ident => usedVars.add(ident));

    // Mixins
    declaredMixins.push(...gatherDeclaredMixins(parseTree).map(objectWithPath));
    gatherUsedMixins(parseTree).forEach(ident => usedMixins.add(ident));

    // Functions
    declaredFunctions.push(...gatherDeclaredFunctions(parseTree).map(objectWithPath));
    gatherUsedFunctions(parseTree).forEach(ident => usedFunctions.add(ident));
  });

  const unusedVars = declaredVars.filter(ident => !usedVars.has(ident.name));
  const unusedMixins = declaredMixins.filter(ident => !usedMixins.has(ident.name));
  const unusedFunctions = declaredFunctions.filter(ident => !usedFunctions.has(ident.name));
